...
def classify_emotion(keywords: str) -> str:
    prompt = PromptTemplate(
        ...
        사랑(강력한), 사랑(순수한), 사랑(영원한), 사랑(행복한), 사랑(따뜻한),
        슬피(후회), 슬피(이벤), 슬피(그리운), 슬피(위로),
        축호(승진), 축호(개여행), 축호(조로원), 축호(경함), 축호(출산),
        응원(새로운 시작), 응원(학격 기원), 응원(격력), 응원(꿔야할 기본),
        행복(영원한), 행복(순수한), 행복(함께한), 행복(다가올),
        특별함(비밀), 특별함(신ube44), 특별함(마법), 특별한(고귀), 특별한(고급)
...
def expand_keywords(keywords: str) -> str:
    prompt = PromptTemplate(
        ...
        이 키워드를 바탕으로 감정과 상황을 포함한 자연스럽고 모든 의도가 자랑나는 문장으로 확장해줘.
        너무 긴장하지 않고, 말하고자 하는 목적이 빠져나타내게 해줘.
        """
    )
    chain = LLMChain(llm=llm, prompt=prompt)
