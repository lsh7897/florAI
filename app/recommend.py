...
emotion_prompt = PromptTemplate(
    ...
    사랑(강력한), 사랑(순수한), 사랑(영원한), 사랑(행복한), 사랑(담당적인),
    슬프(화해), 슬프(이번), 슬프(그리우름), 슬프(위로),
    축호(승진), 축호(개여행), 축호(학격), 축호(생일), 축호(추천),
    응원(시작), 응원(학격), 응원(격려), 응원(도전),
    행복(영원한), 행복(순수한), 행복(함꿐한), 행복(다가올),
    특별함(비밀), 특별함(신비), 특별함(마보), 특별함(고규), 특별함(고급)
    """
)
